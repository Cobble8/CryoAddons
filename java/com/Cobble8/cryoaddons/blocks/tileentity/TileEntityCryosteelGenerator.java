...
public class TileEntityCryosteelGenerator extends TileEntity implements ITickable
{
		...
		if(!handler.getStackInSlot(0).isEmpty() && isItemFuel(handler.getStackInSlot(0)))
			{
				...
				int rfpt = energy += getFuelValue(handler.getStackInSlot(0))/20;
				if(cookTime == 21)
					{
					cookTime = 0;
	...
	{
		if(stack.getItem() == ModItems.CRYOSTEEL_NUGGET) return 100;
		if(stack.getItem() == ModItems.CRYOSTEEL_INGOT) return 900;
		//if(stack.getItem() == ModBlocks.CRYOSTEEL_BLOCK) return 8100;
		else return 0;
	}
	...
	{
		return new TextComponentTranslation("container.cryosteel_generator");
	}
